...
function Landing() {
  ...
  useEffect(() => {
    if (cityId) {
      handleCityById(cityId);
    }
  }, [cityId]);

  async function handleCityById(cityId: string) {
    try {
      const response = await api.get(
        `?id=${cityId}&appid=e81965343df414cbfb25d98c8741fe2a&lang=en&units=metric`
      );
  ...
  async function handleCity(e: FormEvent) {
    ...
    await api
      .get(
        `?q=${city}&appid=e81965343df414cbfb25d98c8741fe2a&lang=en&units=metric`
      )
      .then((response) => {
        setData(response.data);
      })
  ...
  async function handleCityByCordinates(lat: number, lon: number) {
    await api
      .get(
        `?lat=${lat}&lon=${lon}&appid=e81965343df414cbfb25d98c8741fe2a&lang=en&units=metric`
      )
      .then((response) => {
        setValid(true);
        setData(response.data);
    ...
    <div id="main">
      ...
      <div className="main-grid">
        ...
        <div className="credits">
          by&nbsp;
          <a
            href="https://github.com/satchit-r"
            target="_blank"
            rel="noopener noreferrer"
          >
            <strong>Satchit R</strong>
          </a>
        </div>
      </div>
    </div>
